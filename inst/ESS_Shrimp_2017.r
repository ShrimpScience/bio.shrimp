...
#TABLE DATA:
#Number of tows per year the survey has been running
...
opts(plot.background=theme_rect(fill=”yellow”),
     panel.background=theme_rect(fill=’purple’))


	####    PLOT gulf CPUE  
jpeg(filename="gulf_cpue.jpg")
ggplot(Gulf.cpue, aes(YEAR,CPUE)) + geom
...
colours<-rep("yellow", length(Gulf.cpue$GULF_CPUE))
colours[Gulf.cpue$GULF_CPUE<ry]<-"red"
colours[Gulf.cpue$GULF_CPUE>yg]<-"green"
plot(Gulf.cpue$YEAR,Gulf.cpue$GULF_CPUE, ylab="", xlab="", col=colours, pch=19)
lines(Gulf.cpue$YEAR,Gulf.cpue$GULF_CPUE)
abline(h=ry, col="red")
